...
- **Template composer assembly done 2026-07-23** — private/unpublished draft at `railway.com/deploy/nD9a5I`. Key finding: **the composer does not read this repo's `railway.json`** — it's a deploy-time file, not a composition-time one. The healthcheck path (`/health`) had to be set again by hand in the composer's own Settings tab. `DATABASE_URL` has no stock value on the `pgvector/pgvector` image (confirmed via the composer's reference-variable autocomplete, which only offered `POSTGRES_USER`/`POSTGRES_DB`/`POSTGRES_PASSWORD`) — built manually as `postgres://${{Postgres.POSTGRES_USER}}:${{Postgres.POSTGRES_PASSWORD}}@${{Postgres.RAILWAY_PRIVATE_DOMAIN}}:5432/${{Postgres.POSTGRES_DB}}`. Worth carrying forward to the next template built this way.
- **License: MIT, decided 2026-07-23.** `LICENSE` file added, `package.json`'s `license` field set, noted in `README.md`. Unblocks Open Source Partner Program eligibility (gated on this per the best-practices doc).
- **Marketplace listing copy: `MARKETPLACE.md`, written 2026-07-23**, following Railway's documented H1/H2/H3 structure (`docs.railway.com/guides/templates-best-practices`) — H1 intro, "About Hosting," "Common Use Cases," "Dependencies," "Deployment Dependencies," an optional implementation-details snippet (reused the verified graph-expansion example from §2 above), and "Why Deploy on Railway." Separate file from `README.md` (dev quickstart) since this is the composer's "Info" field content, a different audience. Uses "Switchyard" throughout — same placeholder-name caveat as everywhere else, swap on rename.
- **Icon: only the template-level icon needs a custom asset, confirmed 2026-07-22.** Checked `devicons.railway.com` directly (the source Railway's own "Update Info → Icon" picker pulls from, per `docs.railway.com/services`) — it's a fixed library of existing tech-stack logos (React, Postgres, Docker, etc.), not a custom-upload service. So the two **service** icons (App, Postgres) get picked from that library at template-composer time — no design work needed there. The **template-level** icon (marketplace grid identity) isn't sourced from devicons and needs an original asset: `assets/icon.svg` — a stroke-based fork/junction mark (one trunk splitting into two branches at a highlighted node), which doubles as a literal railway-switch reference and a picture of the app's own architecture (one query splitting into vector-path + graph-path retrieval, reconverging as combined results). Single accent color (`#D97706`, warm amber/signal-lamp, deliberately not Railway's own purple/pink), 1:1 viewBox, no background element — transparency is structural, not something that could have silently broken. Placeholder-tier like the name: swap if it doesn't fit whatever the final Railyard branding turns out to be. Exact upload format/dimensions for the template-level icon specifically weren't confirmed against the live template composer (needs an authenticated dashboard session, out of scope for this pass) — SVG is the safe starting format; rasterize to PNG then if the upload widget requires it.
